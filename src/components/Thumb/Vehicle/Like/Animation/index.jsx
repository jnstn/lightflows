...
const ThumbVehicleLikeAnimation = ({ init, duration }) => {
  ...
  useLayoutEffect(() => {
    if (init) {
      ...
      const navHeart =
        clickedHeart.parentElement.parentElement.parentElement.parentElement.parentElement.querySelector(
          '.nav_primary .nav_primary-liked svg',
        );
      const navRect = navHeart.getBoundingClientRect();
      const clickedRect = clickedHeart.getBoundingClientRect();
      const animY = clickedRect.y - navRect.y;
      const animX = clickedRect.x - navRect.x;
      ...
      tl.to(
        clickedHeart,
        {
          duration: dur,
          x: -animX,
          ease: CustomEase.create('custom', 'M0,0 C0.298,0 1,0.705 1,1 '),
        },
        `start`,
      );
      tl.to(
        clickedHeart,
        {
          duration: dur,
          y: -animY,
          ease: CustomEase.create('custom', 'M0,0 C0,0.302 0.695,1 1,1 '),
        },
        'start',
      );
    }
...
export { ThumbVehicleLikeAnimation };
